...
/**
 * Função responsável abrir o modal das Disciplinas
 */
...
/**
 * Função responsável fechar o modal das Disciplinas
 */
...
/**
 * Função responsável por criar os cards da tabela disciplina
 * @param {nome} string
 ...
 * @param {id} string
 */
 const createDisciplinasTableRow = (nomeDisciplina, cargaHoraria, professor, status, observacoes, idDisciplina ) => {
  ...
  const disciplinas = document.querySelector('#Disciplinas-Cards')
  ...
  <div class="subject-card">
    ...
    <p style="height: 15px">${observacoes}</p>
    <div align="right" style="padding: 50px 0px 30px 0px" >
      <button class="button button--danger" onclick=deleteDisciplinaCard(${idDisciplina})>Apagar</button>
      <button class="button button--success" onclick="editdDisciplinaModal(${idDisciplina})">Editar</button>
    </div>
  </div>`;
  disciplinas.appendChild(divCards);
}
...
/**
 * Função responsável por salvar dados das Discipplinas
 * @param {url} string
 * @param {method} string
 */
...
const createStudent = () => {
  ...
  /**
 * Função responsável abrir o modal das disciplinas e salvar uma nova disciplina
 * @param {id} string
 */
 ...
 */
 const deleteDisciplinaCard = async (idDisciplina)  =>  {

  if (confirm("Tem Certeza Que Deseja Deletar Disciplina?") == true) {
    fetch(`http://localhost:3000/disciplinas/${idDisciplina}`, {method : 'DELETE'});
    alert('Deletado Com Sucesso.');
    location.reload();
  }
}
 ...
 */
 const loadDisciplinasCards = () => {
  ...
  .then(data => {
    data.forEach(item => {
      createDisciplinasTableRow(item.nomeDisciplina, item.cargaHoraria, item.professor, item.status, item.observacoes, item.id)
    })
  }).catch((error) => {
    alert('ocorreu um erro tente mais tarde')
    console.error(error);
